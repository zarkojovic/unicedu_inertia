        ...
        <div class="mx-auto bg-white rounded-xl shadow-md overflow-hidden w-11/12 p-4 lg:px-8">
            ...
            <div class="md:flex md:justify-left">
                <div class="mx-auto md:mx-0 w-36">
                    <img class="h-full w-full object-cover rounded-full" :src="img" alt="Student profile image"/>
                </div>
                <div class="grid grid-col-1 md:ml-8">
                    <p class="text-lg capitalize tracking-wide font-semibold text-center md:text-left self-center">{{ firstName }} {{ lastName }}</p>
                    <p class="mt-1 text-md leading-tight font-medium text-gray-400 text-center md:text-left md:mt-0">{{ email }}</p>
                    ...
                    <form method="POST" enctype="multipart/form-data" class="flex justify-center mt-2" :action="route('user.image.update')">
                        <label class="profile-image-label text-orange-500 md:text-left"
